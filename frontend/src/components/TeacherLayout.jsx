...
const TeacherLayout = () => {
  ...
  const baseRoute = "/dashboard/teacher";

  const navItems = [
    { title: "Dashboard", icon: Home, route: baseRoute },
    { title: "Academic Register", icon: ClipboardList, route: `${baseRoute}/academic-register` },
    { title: "Assigned Subjects", icon: BookCopyIcon, route: `${baseRoute}/assigned-subjects` },
    { title: "Grace Mark", icon: Award, route: `${baseRoute}/grace-mark` },
    { title: "Student Attendance", icon: ClipboardList, route: `${baseRoute}/std-attendance-register` },
    ...
    <div className="flex h-screen bg-[#11322f] font-sans overflow-hidden">
      ...
      >
        ...
        <div className="px-4 py-3 border-t border-[#0d2522] relative" ref={profileMenuRef}>
          ...
          >
            <img 
              src={authUser?.profileImage || authUser?.avatar || "https://ui-avatars.com/api/?name=" + (authUser?.name || "Teacher") + "&background=00b87c&color=fff"} 
              alt="User" 
              className="w-8 h-8 rounded-full bg-[#0d2522] object-cover"
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-white truncate">{authUser?.name || "Teacher"}</p>
              <p className="text-[10px] text-brand-mint/80 truncate mt-0.5">{authUser?.email || "Teacher"}</p>
            </div>
          </button>
      ...
      <div className="lg:hidden fixed bottom-2 left-3 right-3 bg-[#0d2522] rounded-2xl px-6 py-3.5 flex justify-between items-center z-30 shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-white/5">
        ...
        <Link to={`${baseRoute}/assigned-subjects`} className={`flex flex-col items-center gap-1 transition-colors ${location.pathname.includes('/assigned-subjects') ? 'text-brand-mint' : 'text-gray-400 hover:text-gray-300'}`}>
          <BookCopyIcon size={22} className={location.pathname.includes('/assigned-subjects') ? 'fill-brand-mint/20' : ''} />
