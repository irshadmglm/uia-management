...
const StudentLayout = () => {
  ...
  const navItems = [
    { title: "Dashboard", icon: Home, route: baseRoute },
    { title: "Academic Register", icon: ClipboardList, route: `${baseRoute}/academic-register` },
    ...
    ...(batch?.CEmarkList ? [{ title: "CE Mark", icon: BookMarked, url: batch.CEmarkList }] : []),
    ...
    <div className="flex h-screen bg-[#11322f] font-sans overflow-hidden">
      ...
      >
        ...
        <div className="px-4 py-3 border-t border-[#0d2522] relative" ref={profileMenuRef}>
          ...
          >
            <img 
              src={authUser?.profileImage || authUser?.avatar || "https://ui-avatars.com/api/?name=" + (authUser?.name || "Student") + "&background=00b87c&color=fff"} 
              alt="User" 
              className="w-8 h-8 rounded-full bg-[#0d2522] object-cover"
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-white truncate">{authUser?.name || "Student"}</p>
              <p className="text-[10px] text-brand-mint/80 truncate mt-0.5">
                {authUser?.cicNumber ? `CIC: ${authUser.cicNumber}` : (authUser?.role || 'Student')}
              </p>
            </div>
          </button>
      ...
      <div className="lg:hidden fixed bottom-2 left-3 right-3 bg-[#0d2522] rounded-2xl px-6 py-3.5 flex justify-between items-center z-30 shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-white/5">
        <Link to={baseRoute} className={`flex flex-col items-center gap-1 transition-colors ${location.pathname === baseRoute ? 'text-brand-mint' : 'text-gray-400 hover:text-gray-300'}`}>
          <Home size={22} className={location.pathname === baseRoute ? 'fill-brand-mint/20' : ''} />
          <span className="text-[10px] font-semibold">Home</span>
        </Link>
        <Link to={`${baseRoute}/subjects`} className={`flex flex-col items-center gap-1 transition-colors ${location.pathname.includes('/subjects') ? 'text-brand-mint' : 'text-gray-400 hover:text-gray-300'}`}>
          <BookCopyIcon size={22} className={location.pathname.includes('/subjects') ? 'fill-brand-mint/20' : ''} />
          <span className="text-[10px] font-semibold">Subjects</span>
        </Link>
        <Link to={`${baseRoute}/semester-list`} className={`flex flex-col items-center gap-1 transition-colors ${location.pathname.includes('/semester') ? 'text-brand-mint' : 'text-gray-400 hover:text-gray-300'}`}>
          <ListChecks size={22} className={location.pathname.includes('/semester') ? 'fill-brand-mint/20' : ''} />
          <span className="text-[10px] font-semibold">Marks</span>
        </Link>
        <Link to={`${baseRoute}/profile`} className={`flex flex-col items-center gap-1 transition-colors ${location.pathname.includes('/profile') ? 'text-brand-mint' : 'text-gray-400 hover:text-gray-300'}`}>
          <User size={22} className={location.pathname.includes('/profile') ? 'fill-brand-mint/20' : ''} />
